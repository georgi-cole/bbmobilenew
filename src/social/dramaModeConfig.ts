...
export const DRAMA_SOCIAL_ACTIONS: SocialActionDefinition[] = [
  ...
  {
    id: 'private_flirt', title: 'Flirt in Private', icon: '\uD83D\uDC8B',
    description: 'Nudge a growing spark away from the group.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 1 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['spark', 'building'],
  },
  {
    id: 'late_night_talk', title: 'Late-Night Talk', icon: '\uD83C\uDF19',
    description: 'Open up when the rest of the house is asleep.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 2 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['spark', 'building', 'established'],
  },
  {
    id: 'cuddle', title: 'Cuddle', icon: '\uD83E\uDD17',
    description: 'Share a close private moment under the blankets.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 1 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['established', 'climax'],
  },
  {
    id: 'kiss_under_covers', title: 'Secret Kiss', icon: '\uD83D\uDC8B',
  ...
  {
    id: 'pool_makeout', title: 'Pool Makeout', icon: '\uD83C\uDF0A',
    description: 'Turn the chemistry up with very little cover.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 3 },
    targetMode: 'primary', successWeight: 1, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['established', 'climax'],
  },
  {
    id: 'spend_night', title: 'Spend the Night', icon: '\uD83D\uDECF\uFE0F',
    description: 'Choose intimacy over subtlety and accept the risk.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 4 },
    targetMode: 'primary', successWeight: 1, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['climax'],
  },
  {
    id: 'go_public', title: 'Go Public', icon: '\uD83D\uDCE3',
    description: 'Stop hiding the romance and face the strategic fallout.',
    category: 'strategic', kind: 'political_spend', baseCost: { energy: 3, influence: 1 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'romance', dramaOnly: true,
    requiredArcTypes: ['romance'], requiredArcStages: ['established', 'climax'],
    requiredArcPublic: false,
  },
  {
    id: 'end_romance', title: 'End It', icon: '\uD83D\uDC94',
    description: 'Break off the relationship before the game does it for you.',
    category: 'aggressive', kind: 'aggressive', baseCost: { energy: 2 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'ex', dramaOnly: true,
    requiredArcTypes: ['romance'],
    requiredArcStages: ['building', 'established', 'strained', 'climax'],
  },
  {
    id: 'break_alliance', title: 'Break the Pact', icon: '\uD83D\uDDE1\uFE0F',
    description: 'End an alliance and accept the memory it leaves behind.',
    category: 'aggressive', kind: 'aggressive', baseCost: { energy: 3 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'betrayal', dramaOnly: true,
    requiredRelationshipTags: ['alliance'],
  },
  {
    id: 'snoop_around', title: 'Snoop Around', icon: '\uD83D\uDD75\uFE0F',
    description: 'Look for private deals, hidden chemistry, or loose receipts.',
    category: 'strategic', kind: 'intel_gain', baseCost: { energy: 2 },
    targetMode: 'none', needsTargets: false, successWeight: 2, yields: { info: 2 },
    dramaOnly: true,
  },
  {
    id: 'ride_or_die',
  ...
  {
    id: 'risk_the_vibe', title: 'Risk the Vibe', icon: '\uD83C\uDFB2',
    description: 'See whether a ride-or-die friendship is becoming something more.',
    category: 'friendly', kind: 'rapport', baseCost: { energy: 2 },
    targetMode: 'primary', successWeight: 1, outcomeTag: 'romance_seed', dramaOnly: true,
    requiredArcTypes: ['bromance'], requiredArcStages: ['building', 'established', 'climax'],
    excludedArcTypes: ['romance'],
  },
  {
    id: 'break_bromance', title: 'End the Pact', icon: '\u2702\uFE0F',
    description: 'Tell your ride-or-die the partnership is over.',
    category: 'aggressive', kind: 'aggressive', baseCost: { energy: 3 },
    targetMode: 'primary', successWeight: 2, outcomeTag: 'betrayal', dramaOnly: true,
    requiredArcTypes: ['bromance'], requiredArcStages: ['building', 'established', 'strained', 'climax'],
  },
  {
    id: 'plant_lie',
    title: 'Plant a Lie',
...
export const DRAMA_DIALOGUE_BANK: {
  ...
  rumour: {
    ...
    secret_romance: ['{source} says {subject} keeps disappearing with the same person after lights-out.'],
...
DRAMA_DIALOGUE_BANK.rumour.secret_alliance = [...DRAMA_DIALOGUE_BANK.rumour.secret_alliance, '{source} heard that {subject} has a final deal hidden inside a larger alliance.', '{source} claims {subject} meets the same person after every vote.'];
DRAMA_DIALOGUE_BANK.rumour.secret_romance = [...DRAMA_DIALOGUE_BANK.rumour.secret_romance, '{source} spotted {subject} sharing a suspiciously private late-night moment.', '{source} heard that {subject} is hiding feelings that could change the game.'];
DRAMA_DIALOGUE_BANK.rumour.targeting = [...DRAMA_DIALOGUE_BANK.rumour.targeting, '{source} says {subject} is the name power players keep testing.', '{source} heard that {subject} is the backup plan nobody wants to own.'];
DRAMA_DIALOGUE_BANK.rumour.fake_deal = [...DRAMA_DIALOGUE_BANK.rumour.fake_deal, '{source} compared promises and says {subject} reused the exact same pitch.', '{source} believes {subject} has promised safety to both sides.'];
DRAMA_DIALOGUE_BANK.rumour.personal_comment = [...DRAMA_DIALOGUE_BANK.rumour.personal_comment, '{source} says {subject} got personal when the cameras moved away.', '{source} heard that {subject} made a joke that landed like an insult.'];
DRAMA_DIALOGUE_BANK.exposure.secret_alliance = [...DRAMA_DIALOGUE_BANK.exposure.secret_alliance, 'HOUSE EXPOSED: Receipts reveal {subject} has been protecting a hidden partner.', 'HOUSE EXPOSED: The secret voting arrangement around {subject} has reached the living room.'];
DRAMA_DIALOGUE_BANK.exposure.secret_romance = [...DRAMA_DIALOGUE_BANK.exposure.secret_romance, 'HOUSE EXPOSED: A hidden kiss involving {subject} is no longer secret.', 'HOUSE EXPOSED: The late-night chemistry around {subject} has reached the whole house.'];
DRAMA_DIALOGUE_BANK.exposure.targeting = [...DRAMA_DIALOGUE_BANK.exposure.targeting, 'HOUSE EXPOSED: Multiple private pitches named {subject} as the next move.', 'HOUSE EXPOSED: {subject} was the backup plan in more rooms than anyone admitted.'];
DRAMA_DIALOGUE_BANK.exposure.fake_deal = [...DRAMA_DIALOGUE_BANK.exposure.fake_deal, 'HOUSE EXPOSED: Two promises from {subject}; one impossible vote.', 'HOUSE EXPOSED: {subject} gave both sides the same guarantee.'];
DRAMA_DIALOGUE_BANK.exposure.personal_comment = [...DRAMA_DIALOGUE_BANK.exposure.personal_comment, 'HOUSE EXPOSED: The private remark involving {subject} has become public ammunition.', 'HOUSE EXPOSED: The comment involving {subject} has split the house.'];
