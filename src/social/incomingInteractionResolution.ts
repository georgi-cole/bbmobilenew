...
function fallbackScenarioOutcome(
  ...
  const replies: Record<string, string> = {
    week_start_ally_check_in:
      `${fromName} says they want to compare notes before the week gets away from them.`,
    ...
    safety_win_congratulations:
      `${fromName} says your Safety win changed who can take risks this week.`,
    player_nominated_support:
      `${fromName} says the block has made every friendly face harder to read.`,
    player_nominated_tension:
      `${fromName} says the nomination changed how they see your relationship.`,
    competition_low_finish_support: `${fromName} says one weak result does not define your week.`,
    competition_low_finish_taunt:
      `${fromName} says the result showed exactly who is under pressure.`,
    social_momentum_notice:
      `${fromName} says people have started comparing notes about your social game.`,
    hoh_safety_request: `${fromName} says safety from you would change their whole week.`,
    nominee_hoh_plea:
      `${fromName} says staying off the block keeps one more vote within your reach.`,
    nominee_veto_pitch:
      `${fromName} says using Safety on them would force the house to recalculate.`,
    nominee_campaign: `${fromName} says the vote is still movable, but they need another number.`,
    nomination_aftershock:
      `${fromName} says the nomination changed who they think is really with them.`,
    nominee_understands_loh:
      `${fromName} says they understand the move, but will remember who made it.`,
    nominee_confronts_loh:
      `${fromName} says the nomination felt personal, whatever the strategy was.`,
    replacement_nominee_reacts_to_loh:
      `${fromName} says the replacement choice changed how they see you.`,
    post_veto_gratitude:
      `${fromName} says the Safety decision bought them time they will remember.`,
    post_veto_campaign:
      `${fromName} says the new block has reopened votes they thought were settled.`,
    ...
    betrayal_warning:
      `${fromName} says the same suspicious pattern has come up too often to ignore.`,
    ignored_warning:
      `${fromName} says the distance between you is starting to affect their choices.`,
    targeted_snark: `${fromName} says they wanted to see whether the jab would get a reaction.`,
    alliance_reassurance:
      `${fromName} says the alliance needs one clear move to prove it still exists.`,
    generic_gossip:
      `${fromName} says ${target} is being discussed, but the claim is still incomplete.`,
    generic_check_in: `${fromName} says they are still trying to read where they stand with you.`,
    relationship_friendship_check_in:
      `${fromName} says the friendship feels more real than they expected.`,
    relationship_alliance_follow_up:
      `${fromName} says they need to know whether you are actually working together.`,
    relationship_romance_check_in:
      `${fromName} says they do not want the connection to become house gossip.`,
    relationship_confidant_check_in:
      `${fromName} says they have something private to share if they can trust you.`,
    relationship_frustration_follow_up:
      `${fromName} says the mixed signals are affecting how they play around you.`,
    relationship_repair_follow_up:
      `${fromName} says fixing this will take more than one good conversation.`,
  }
