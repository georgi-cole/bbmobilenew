...
describe('evaluateSocialActionEligibility', () => {
  ...
  it('keeps advanced contextual gates out of Normal Mode', () => {
    expect(
      evaluateSocialActionEligibility({
  ...
  it('requires an active alliance before a Drama Mode betrayal', () => {
    ...
    expect(evaluateSocialActionEligibility({ ...base, relationships: { user: { ally: { affinity: 30, tags: [] } } } }).eligible).toBe(false);
    expect(evaluateSocialActionEligibility({ ...base, relationships: { user: { ally: { affinity: 30, tags: ['alliance'] } } } }).eligible).toBe(true);
  });
  ...
  it('enforces role and weekly window for Drama Mode political actions', () => {
    ...
    expect(evaluateSocialActionEligibility({ ...base, targetIds: ['loh'], phase: 'social_1' }).eligible).toBe(true);
    expect(evaluateSocialActionEligibility({ ...base, targetIds: ['ally'], phase: 'social_1' }).eligible).toBe(false);
    expect(evaluateSocialActionEligibility({ ...base, targetIds: ['loh'], phase: 'live_vote' }).eligible).toBe(false);
  });
