...
const RESPONSE_LOG_COPY: Record<IncomingInteractionResponseType, readonly string[]> = {
  positive: ['You met {name} halfway, and they noticed.', 'You gave {name} the response they hoped for.', 'You leaned into the conversation with {name}.', 'You turned the approach from {name} into genuine goodwill.'],
  neutral: ['You heard {name} out without showing your whole hand.', 'You kept things measured with {name}.', 'You gave {name} an answer, but no guarantee.', 'You left the door open without promising {name} anything.'],
  negative: ['You pushed back and {name} did not hide their reaction.', 'You drew a hard line with {name}.', 'You challenged the story from {name}.', 'You made it clear to {name} that the pitch did not land.'],
  accept: ['You accepted the offer from {name} and made the deal real.', 'You shook on it with {name}; now the promises count.', 'You told {name} yes, and raised the stakes for both of you.', 'You joined the plan from {name} with cameras watching.'],
  decline: ['You refused the offer from {name} and closed that strategic door.', 'You told {name} no without leaving much room for repair.', 'You rejected the deal; {name} will remember it.', 'You chose distance over the arrangement {name} offered.'],
  dismiss: ['You ended the conversation before {name} could finish the pitch.', 'You gave {name} nothing to work with.', 'You walked away from the approach by {name}.', 'You left {name} without an answer.'],
  ignore: ['You let the message from {name} expire unanswered.', 'You never answered {name}, and the silence became the answer.', 'You ignored the approach by {name} until the moment passed.', 'You left {name} waiting through the end of the week.'],
};
