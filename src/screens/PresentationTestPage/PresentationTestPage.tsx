  ...
  return (
    <main className="presentation-test">
      <header className="presentation-test__header">
        ...
        <p>Preview the nominee indication and premium Faux-TV weather artwork without starting a season.</p>
      </header>
        ...
        <div className={`presentation-test__roster${showNomineeGlow ? ' evictionDrama' : ''}`}>
          {DEMO_PLAYERS.map((player, index) => (
            <article
              ...
              data-eviction-nominee={showNomineeGlow && (index === 0 || index === 2) ? 'true' : undefined}
              key={player.name}
            >
              <div className="presentation-test__portrait" style={{ background: `linear-gradient(145deg, ${player.color}, #17111f 70%)` }}>
      ...
      <section className="presentation-test__section presentation-test__weather">
        <div className="presentation-test__section-head">
          ...
          <select value={weather} onChange={(event) => setWeather(event.target.value as WeatherConditionId)} aria-label="Weather state">
            {WEATHER_STATES.map((state) => <option key={state} value={state}>{state.replace('_', ' ')}</option>)}
          </select>
        </div>
        <div className={`presentation-test__tv weather-tv-card--${weather}`}>
          <div className="presentation-test__weather-main">
            <span className="presentation-test__temperature">22<sup>°C</sup></span>
            <div className="presentation-test__weather-icon">
              <WeatherGlyph condition={weather} rainbow={false} />
              <strong>{weather.replace('_', ' ')}</strong>
            </div>
          </div>
          <p className="presentation-test__copy">The premium weather artwork now used by the Faux-TV card.</p>
        </div>
      </section>
    </main>
  )
