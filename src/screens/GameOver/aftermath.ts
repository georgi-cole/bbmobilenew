...
function validateLinkedScenario(
  ...
  if (typeof value.relation !== 'string' || !RELATIONS.includes(value.relation as AftermathRelationKind)) {
...
function listTabloidPhotos(): TabloidPhotoEntry[] {
  return Object.keys(TABLOID_PHOTO_MODULES)
    ...
    .sort((left, right) => {
      if (left.matchToken !== right.matchToken) return left.matchToken.localeCompare(right.matchToken);
...
function relationFromAffinity(
  ...
  const entries = Object.entries(social?.relationships?.[playerId] ?? {})
    .filter(([targetId]) => targetId !== playerId && players.some((player) => player.id === targetId))
    .sort((left, right) => (
      direction === 'highest'
...
function buildContext(
  ...
  if (alliance && options.social?.dramaNetwork?.alliances.some(
...
function isEligible(scenario: AftermathScenario, context: AftermathPlayerContext): boolean {
  ...
  if (eligibility.placementMin !== undefined && context.placement < eligibility.placementMin) return false;
  if (eligibility.placementMax !== undefined && context.placement > eligibility.placementMax) return false;
  if (eligibility.tagsAny?.length && !eligibility.tagsAny.some((tag) => context.tags.has(tag))) return false;
  if (eligibility.tagsAll?.length && !eligibility.tagsAll.every((tag) => context.tags.has(tag))) return false;
...
function scenarioWeight(
  ...
  const matchedTags = scenario.eligibility.tagsAny?.filter((tag) => context.tags.has(tag)).length ?? 0;
  weight *= 1 + matchedTags * 0.7;

  if (
    (context.tags.has('winner') || context.tags.has('public_favorite') || context.tags.has('fan_favorite'))
...
function renderTemplate(
  ...
  const values: Record<string, string> = {
    ...
    object: 'them',
    possessive: 'their',
    placement: context.placement === Number.MAX_SAFE_INTEGER ? context.placementLabel : String(context.placement),
    ...
    winnerName,
    publicApproval: context.publicApproval === null ? 'unrecorded' : `${Math.round(context.publicApproval)}%`,
  };

  return template
    .replace(/\{([A-Za-z][A-Za-z0-9]*)\}/g, (_match, placeholder: string) => values[placeholder] ?? '')
    .replace(/\s{2,}/g, ' ')
    .replace(/\s+([,.;!?])/g, '$1')
    .trim();
}
...
function chooseTwo(values: string[], seed: string): [string, string] {
  ...
  if (values.length > 1 && secondIndex === firstIndex) secondIndex = (secondIndex + 1) % values.length;
...
function buildLinkedStories(
  ...
  relationPriority.forEach((relation) => {
    ...
    const pair = linkedPairCandidates(contexts, relation)
      .filter(([left, right]) => !usedPlayers.has(left.player.id) && !usedPlayers.has(right.player.id))
      .sort(([leftA, rightA], [leftB, rightB]) => (
        `${leftA.player.id}:${rightA.player.id}`.localeCompare(`${leftB.player.id}:${rightB.player.id}`)
      ))[0];
    if (!pair) return;
...
export function buildAftermathIssue(
  ...
  const winnerName = contexts.find((context) => context.placement === 1)?.player.name ?? 'the winner';
  ...
  linkedStories.forEach((story) => {
    ...
    const categoryEntry = Object.entries(config.categories)
      .find(([, label]) => label === story.categoryLabel)?.[0];
    if (categoryEntry) categoryCounts.set(categoryEntry, (categoryCounts.get(categoryEntry) ?? 0) + 1);
  });
  ...
  const stories = contexts.map((context) => {
    ...
    const withoutDuplicates = eligible.filter(
      (scenario) => !usedScenarioIds.has(scenario.id) && !usedCooldownGroups.has(scenario.cooldownGroup),
    );
    const withoutScenarioDuplicates = eligible.filter((scenario) => !usedScenarioIds.has(scenario.id));
    const candidates = withoutDuplicates.length > 0
      ? withoutDuplicates
    ...
    const scenario = weightedPick(
      candidates,
      `${seed}:scenario`,
      (candidate) => scenarioWeight(candidate, context, toneCounts, categoryCounts),
    );
