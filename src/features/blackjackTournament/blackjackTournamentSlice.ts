...
function simulateAiFinals(state: BlackjackTournamentState): void {
  while (state.remainingPlayerIds.length > 1) {
    const controllerIndex = Math.floor(rngAt(
      (state.seed ^ Math.imul(state.duelIndex + 1, 0x9e3779b9)) >>> 0,
      0,
    ...
    while (true) {
      ...
      if (rematch >= REMATCH_CAP) {
        const fallback = rematchCapWinner(state.seed, state.duelIndex, pair.fighterAId, pair.fighterBId, rematch);
    ...
    state.playerScores[loserId] = Math.max(0, (state.playerScores[loserId] ?? STARTING_PLAYER_SCORE) - 1);
