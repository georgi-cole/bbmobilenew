...
export function buildQuickTapRaceLayout(
  ...
  const laneGap = Math.max(10, trackRect.height * 0.03);
  const laneHeight = Math.max(44, (trackRect.height - laneGap * (laneCount - 1)) / laneCount);
  const racerRadius = Math.max(12, Math.min(20, laneHeight * 0.28));
