...
import { getAvatarUrls } from '../components/AssetPreloaderOverlay/AssetPreloaderOverlay'
...
describe('presentation avatar assets', () => {
  ...
  it('does not rewrite non-canonical or already-grey assets', () => {
    expect(resolvePresentationAvatar('/assets/skins/Finn_avatar.png')).toBe('/assets/skins/Finn_avatar.png')
    expect(resolvePresentationAvatar('/assets/skins/backup-grey-lux/Finn_avatar.webp')).toBe('/assets/skins/backup-grey-lux/Finn_avatar.webp')
  })

  it('includes grey WebP and PNG candidates in gameplay preload URLs', () => {
    const urls = getAvatarUrls()
    expect(urls.some((url) => url.includes('/backup-grey-lux/') && url.endsWith('_avatar.webp'))).toBe(true)
    expect(urls.some((url) => url.includes('/backup-grey-lux/') && url.endsWith('_avatar.png'))).toBe(true)
  })
})
