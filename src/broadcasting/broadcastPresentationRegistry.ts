...
export const BROADCAST_PRESENTATION_REGISTRY: Readonly<Record<string, BroadcastPresentationRule>> = {
  ...
  vox_final3_result: {
    playThrough: true,
    announcement: liveCard('Final Three Result', 'The final immunity journey takes another turn.'),
  },
  ...
  coup_detat: {
    ...
    shock: true,
    playThrough: true,
    announcement: liveCard('Detox!', 'Both nominees cleared. Holder names two backup nominees. ⚡'),
  },
  spotlight_veto: {
    eventMajor: true,
    shock: true,
...
export function isRecognizedBroadcastMajorKey(key: unknown): key is string {
  return (
    typeof key === 'string' && getBroadcastPresentationRule(key)?.eventMajor === true
  )
}
