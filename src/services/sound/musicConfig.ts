...
export interface MusicConfigOverrides {
  ...
  modePhaseOverrides?: Partial<
    Record<GameMode, Partial<Record<Phase, MusicSelection>>>
  >;
 ...
 */
export const DEFAULT_MINIGAME_CATEGORY_MUSIC: Readonly<
  Record<GameCategory, MusicSelection>
> = {
...
export function createMusicConfig(overrides: MusicConfigOverrides = {}): MusicConfigDocument {
  return {
    ...
    minigameProfiles:
      overrides.minigameProfiles ?? DEFAULT_MUSIC_CONFIG.minigameProfiles,
...
export function resolveMusicCue(
  ...
  if (isGameOverHash(context.routeHash)) {
    const routeCue = resolveSelection(
      config.contextMusic.gameOver,
      'context.game-over',
      'route',
    );
  ...
  if (context.socialOpen) {
    const socialCue = resolveSelection(
      config.contextMusic.social,
      'context.social',
      'social',
    );
