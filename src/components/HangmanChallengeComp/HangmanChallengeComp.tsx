...
function findRandomHiddenLetters(word: string, guessedLetters: string[], revealedLetters: string[], amount: number, rng: () => number): string[] {
  const guessed = new Set([...guessedLetters, ...revealedLetters].map((letter) => letter.toUpperCase()));
  const hiddenUnique = Array.from(new Set(normalizeWord(word).split('').filter((char) => isLetter(char) && !guessed.has(char))));
  const chosen: string[] = [];
...
function chooseDisabledLetters(alphabet: string[], guessedLetters: string[], disabledLetters: string[], rng: () => number): string[] {
...
export default function HangmanChallengeComp({
  ...
  const badgeLabels = useMemo(() => {
    ...
    return labels;
  }, [roundState.activeTimedEffects, roundState.hiddenRiskArmed, roundState.nextWrongDouble, roundState.nextWrongExtraPenalty, roundState.shieldNextWrong, roundState.waiveNextPenalty]);
    ...
    const interval = setInterval(() => {
      setRoundState((prev) => {
        const nextEffects = prev.activeTimedEffects
          .map((effect) => ({ ...effect, remainingMs: Math.max(0, effect.remainingMs - TIMER_STEP_MS) }))
        ...
        const penaltyRate = nextEffects.some((effect) => effect.id === 'higher_time_penalty') ? 6 : 4;
        const speed = frozen ? 0 : doubled ? 2 : slowed ? 0.75 : 1;
        const deltaElapsed = TIMER_STEP_MS * speed;
        const keepDisabled = nextEffects.some((effect) => effect.id === 'disable_keyboard') ? prev.disabledLetters : [];
        return {
          ...prev,
  ...
  const chooseMysteryEffect = useCallback((): MysteryBoxDefinition => {
    ...
    const targetCategory = categoryRoll < 0.5 ? 'positive' : categoryRoll < 0.8 ? 'tradeoff' : 'cripple';
    ...
    eligible = eligible.filter((effect) => {
      ...
      if (effect.id === 'freeze_timer' && (activeKinds.has('freeze_timer') || activeKinds.has('double_speed'))) return false;
      if (effect.id === 'slow_timer' && (activeKinds.has('slow_timer') || activeKinds.has('double_speed'))) return false;
      if (effect.id === 'higher_time_penalty' && activeKinds.has('higher_time_penalty')) return false;
  ...
  const openMysteryBox = useCallback(() => {
    ...
    const hiddenLetters = () => findRandomHiddenLetters(currentWord.text, roundState.guessedLetters, roundState.revealedLetters, 2, rngRef.current);
    ...
    setRoundState((prev) => {
      ...
      switch (effect.id) {
        case 'reveal_one': {
          revealed.push(...findRandomHiddenLetters(currentWord.text, prev.guessedLetters, prev.revealedLetters, 1, rngRef.current));
        ...
        case 'freeze_timer': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'freeze_timer'), { id: 'freeze_timer', label: 'Timer frozen', remainingMs: FREEZE_MS }];
        ...
        case 'vowel_scan': {
          const hiddenVowelsExist = normalizeWord(currentWord.text)
            .split('')
            .some((char) => 'AEIOU'.includes(char) && !prev.guessedLetters.includes(char) && !prev.revealedLetters.includes(char));
        ...
        case 'slow_timer': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'slow_timer'), { id: 'slow_timer', label: 'Clock slowed', remainingMs: SLOW_MS }];
        ...
        case 'freeze_breaks_perfect': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'freeze_timer'), { id: 'freeze_timer', label: 'Cold pause', remainingMs: FREEZE_TRADEOFF_MS }];
        ...
        case 'remove_two_errors_minus_100': {
          ...
          nextState.wrongLetters = prev.wrongLetters.slice(0, Math.max(0, prev.wrongLetters.length - 2));
        ...
        case 'double_speed': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'double_speed'), { id: 'double_speed', label: 'Clock doubled', remainingMs: DOUBLE_MS }];
          log('Mystery Box: timer speed doubled');
          break;
        }
        case 'disable_keyboard': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'disable_keyboard'), { id: 'disable_keyboard', label: 'Signal jam', remainingMs: DISABLE_KEYBOARD_MS }];
          nextState.disabledLetters = chooseDisabledLetters(ALPHABET, [...prev.guessedLetters, ...prev.wrongLetters], prev.disabledLetters, rngRef.current);
          log('Mystery Box: 3 letters disabled briefly');
          break;
        }
        case 'distort_used': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'distort_used'), { id: 'distort_used', label: 'Static wash', remainingMs: DISTORT_MS }];
          log('Mystery Box: used-letter display distorted');
          break;
        }
        case 'higher_time_penalty': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'higher_time_penalty'), { id: 'higher_time_penalty', label: 'Penalty surge', remainingMs: TIME_PENALTY_SURGE_MS }];
          log('Mystery Box: time penalty increased');
          break;
        }
        case 'lock_boxes': {
          nextState.activeTimedEffects = [...prev.activeTimedEffects.filter((entry) => entry.id !== 'lock_boxes'), { id: 'lock_boxes', label: 'Boxes locked', remainingMs: BOX_LOCK_MS }];
  ...
  }, [boxesLocked, chooseMysteryEffect, currentWord, phase, roundState.guessedLetters, roundState.revealedLetters, roundState.visibleBox]);

  const guessLetter = useCallback((letter: string) => {
    if (phase !== 'playing') return;
    if (roundState.guessedLetters.includes(letter) || roundState.wrongLetters.includes(letter) || roundState.disabledLetters.includes(letter)) {
      return;
    }
  ...
  const finishRound = useCallback((roundSolved: boolean) => {
    ...
    const multiplierPenalty = roundState.scoreMultiplier < 1
      ? Math.round((baseBreakdown.baseScore + baseBreakdown.errorPenalty + baseBreakdown.timePenalty + baseBreakdown.mysteryAdjustments.reduce((sum, item) => sum + item.value, 0) + baseBreakdown.bonuses.reduce((sum, item) => sum + item.value, 0)) * (roundState.scoreMultiplier - 1))
      : 0;
    ...
    if (multiplierPenalty !== 0) finalAdjustments.push({ label: 'Score cut', value: multiplierPenalty });
    if (hiddenRiskPenalty !== 0) finalAdjustments.push({ label: 'Hidden risk', value: hiddenRiskPenalty });
    ...
    const simulateAiRound = (participant: MinigameParticipant): RoundSummary => {
      const localSeed = (seed ^ hashString(`${participant.id}-${roundIndex}-${currentWord.text}`)) >>> 0;
      ...
      const elapsedSecondsAi = aiSolved
        ? Math.round(clamp(14 + currentWord.difficulty * 5 + (1 - skill) * 22 + localRng() * 12, 11, 62))
        : Math.round(clamp(32 + currentWord.difficulty * 5 + localRng() * 18, 24, 78));
      const errors = aiSolved
        ? Math.round(clamp((1 - skill) * 4 + currentWord.difficulty - 1 + localRng() * 2, 0, MAX_ERRORS - 1))
        : Math.round(clamp(3 + currentWord.difficulty * 0.8 + localRng() * 2.5, 2, MAX_ERRORS));
      const revealedRatioAi = aiSolved ? 1 : clamp(0.18 + skill * 0.35 + localRng() * 0.2, 0.12, 0.74);
    ...
    const nextTotals = Object.fromEntries(nextLeaderboard.map((entry) => [entry.participantId, entry.cumulativeScore]));
    ...
    setRoundState((prev) => ({ ...prev, visibleBox: null, boardFlash: roundSolved ? 'solved' : 'failed' }));
    setPhase(roundIndex >= TOTAL_ROUNDS - 1 ? 'final' : 'breakdown');
  }, [cumulativeScores, currentWord, humanParticipant, resolvedParticipants, revealRatio, roundIndex, roundState, seed]);
  ...
  const handleAlphabetPress = useCallback((letter: string) => {
    if (roundState.guessedLetters.includes(letter) ||
        roundState.wrongLetters.includes(letter) ||
        roundState.disabledLetters.includes(letter)) return;
    guessLetter(letter);
    setLetterInput('');
  ...
  const finishCompetition = useCallback(() => {
    ...
    const rawResults = Object.fromEntries(finalRankings.map((entry) => [entry.participantId, entry.cumulativeScore]));
  ...
  const winnerNames = finalRankings.length > 0
    ? finalRankings.filter((entry) => entry.cumulativeScore === finalRankings[0].cumulativeScore).map((entry) => entry.participantName)
    : [];

  return (
    <div className={`hangman-challenge${roundState.boardFlash ? ` hangman-challenge--${roundState.boardFlash}` : ''}`}>
      ...
      <header className="hangman-challenge__header">
        <div>
          ...
          <h2 className="hangman-challenge__title">Round {roundIndex + 1} of {TOTAL_ROUNDS}</h2>
          <p className="hangman-challenge__subtitle">Cumulative score: {cumulativeScores[humanParticipant.id] ?? 0}</p>
        </div>
        <div className={`hangman-challenge__timer${roundState.activeTimedEffects.some((effect) => effect.id === 'freeze_timer') ? ' is-frozen' : ''}${roundState.activeTimedEffects.some((effect) => effect.id === 'double_speed') ? ' is-urgent' : ''}`}>
          <span>Timer</span>
          ...
          <section className="hangman-challenge__playfield">
            ...
            <section className={`hangman-challenge__keyboard-panel${keyboardDistorted ? ' is-distorted' : ''}`} aria-label="Letter entry">
              ...
              <div className="hangman-challenge__alphabet" aria-label="Letter keyboard">
                {ALPHABET.map((letter) => {
                  const attempted = roundState.guessedLetters.includes(letter) || roundState.wrongLetters.includes(letter);
          ...
          {badgeLabels.length > 0 && (
            <div className="hangman-challenge__badges" aria-label="Active modifiers">
              {badgeLabels.map((label) => (
                <span key={label} className="hangman-challenge__badge">{label}</span>
              ))}
            </div>
          )}
      ...
      {phase === 'breakdown' && breakdown && (
        <section className="hangman-challenge__overlay" aria-label="Round breakdown">
          <div className="hangman-challenge__overlay-card">
            ...
            <h3>{displayTokens.every((token) => token !== '•') ? 'Board stabilized' : 'Board blackout'}</h3>
            <div className="hangman-challenge__score-burst">{animatedRoundScore}</div>
            <div className="hangman-challenge__breakdown-list">
              <div><span>Base score</span><strong>{formatAdjustment(breakdown.baseScore)}</strong></div>
              <div><span>Error penalty</span><strong>{formatAdjustment(breakdown.errorPenalty)}</strong></div>
              <div><span>Time penalty</span><strong>{formatAdjustment(breakdown.timePenalty)}</strong></div>
              {breakdown.mysteryAdjustments.map((item) => (
                <div key={`${item.label}-${item.value}`}><span>{item.label}</span><strong>{formatAdjustment(item.value)}</strong></div>
              ))}
              {breakdown.bonuses.map((item) => (
                <div key={`${item.label}-${item.value}`}><span>{item.label}</span><strong>{formatAdjustment(item.value)}</strong></div>
              ))}
              <div className="is-final"><span>Final round score</span><strong>{breakdown.finalRoundScore}</strong></div>
            </div>
            <button className="hangman-challenge__cta" type="button" onClick={() => setPhase('scoreboard')}>
      ...
      {phase === 'scoreboard' && (
        <section className="hangman-challenge__overlay" aria-label="Round scoreboard">
          <div className="hangman-challenge__overlay-card hangman-challenge__overlay-card--scoreboard">
            ...
            <div className="hangman-challenge__scoreboard">
              {roundLeaderboard.map((entry, index) => (
                <article className="hangman-challenge__score-row" key={entry.participantId}>
                  ...
                  <span className={`hangman-challenge__score-status${entry.solved ? ' is-solved' : ' is-failed'}`} aria-label={entry.solved ? 'Solved' : 'Failed'}>
                    {entry.solved ? '✓' : '—'}
                  </span>
            ...
            <button className="hangman-challenge__cta" type="button" onClick={proceedFromScoreboard}>
              Continue
            </button>
          </div>
        </section>
      ...
      {phase === 'final' && (
        <section className="hangman-challenge__overlay hangman-challenge__overlay--final" aria-label="Final results">
          <div className="hangman-challenge__overlay-card hangman-challenge__overlay-card--scoreboard hangman-challenge__overlay-card--final">
            ...
            <div className="hangman-challenge__scoreboard">
              {finalRankings.map((entry, index) => (
                <article className={`hangman-challenge__score-row${entry.cumulativeScore === finalRankings[0]?.cumulativeScore ? ' is-winner' : ''}`} key={entry.participantId}>
                  <div className="hangman-challenge__score-rank">{MEDALS[index] ?? `#${index + 1}`}</div>
                  <span className="hangman-challenge__avatar">{entry.avatarText}</span>
                  <strong className="hangman-challenge__score-name">{entry.participantName}</strong>
                  <strong className="hangman-challenge__final-points">{entry.cumulativeScore}</strong>
                </article>
              ))}
            </div>
