...
function playerAvatarSrc(player: GridPlayer): string {
...
export default function GridOfLuck(props: GenericMinigameProps) {
  ...
  useEffect(() => {
    if (turnMode !== 'resolving') return undefined;
    const timer = setTimeout(() => {
      ...
      setTurnMode('box');
      setRevealState(null);
    }, state.gamePhase === 'final' ? 1100 : 1450);
    return () => clearTimeout(timer);
  }, [state, turnMode]);
  ...
  return (
    <motion.div className={`grid-of-luck${state.gamePhase === 'final' ? ' grid-of-luck--final' : ''}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <motion.div className="grid-of-luck__camera" animate={screenMode === 'zoomIn' ? 'zoomIn' : 'idle'} variants={screenEffects}>
        ...
        <motion.section className="grid-of-luck__players" layout>
          {ranking.map((player, index) => {
            ...
            const avatarSrc = playerAvatarSrc(player);
            ...
            return (
              ...
              >
                ...
                <motion.div className="grid-of-luck__avatar-shell">
                  {avatarSrc.startsWith('http') || avatarSrc.includes('/') ? (
                    <img className="grid-of-luck__avatar" src={avatarSrc} alt={player.name} />
                  ) : (
                    <span className="grid-of-luck__avatar-fallback" aria-hidden="true">{player.avatar}</span>
                  )}
                </motion.div>
