...
function fillDramaLine(line: string, source: string, subject: string): string {
  return line.replaceAll('{a}', source).replaceAll('{b}', subject).replaceAll('{source}', source).replaceAll('{subject}', subject);
}


const ARC_LABEL = { romance: 'Romance', bromance: 'Bromance', rivalry: 'Rivalry', betrayal: 'Betrayal' } as const;
...
export default function HousePulse({ network, players, humanId }: HousePulseProps) {
  ...
  const modal = open ? (
    <div className="house-pulse__overlay" role="presentation" onMouseDown={() => setOpen(false)}>
      <section className="house-pulse__sheet" role="dialog" aria-modal="true" aria-label="House Pulse" onMouseDown={(event) => event.stopPropagation()}>
        <header className="house-pulse__header">
          ...
          <button type="button" onClick={() => setOpen(false)} aria-label="Close House Pulse">&times;</button>
        </header>
        <div className="house-pulse__stats">
          <span><strong>{activeStories}</strong> active stories</span>
          <span><strong>{knownRumours.filter((rumour) => rumour.status === 'circulating').length}</strong> known rumours</span>
          <span><strong>{knownEvents.filter((event) => event.public && event.severity === 'major').length}</strong> shocks</span>
        </div>
        ...
        <div className="house-pulse__content">
          {tab === 'stories' && (knownArcs.length ? knownArcs.map((arc) => {
            const first = arc.participantIds[0] === humanId ? 'You' : playerName(arc.participantIds[0]);
            const second = arc.participantIds[1] === humanId ? 'you' : playerName(arc.participantIds[1]);
            ...
            return (
              <article className={`house-pulse__card house-pulse__card--${arc.type}`} key={arc.id}>
                <div className="house-pulse__card-top"><span>{ARC_LABEL[arc.type]}</span><em>{arc.public ? 'Public' : arc.participantIds.includes(humanId) ? 'Your story' : 'You discovered this'}</em></div>
                <h3>{first} and {second}</h3>
                <p>{story}</p>
              </article>
            );
          }) : <p className="house-pulse__empty">No story has reached your radar yet.</p>)}
          {tab === 'intel' && (knownRumours.length ? knownRumours.map((rumour) => {
            ...
            const isGeneric = !rumour.claim || rumour.claim.startsWith('A private game detail') || rumour.claim.startsWith('A private promise may not match');
            const claim = isGeneric
              ? fillDramaLine(pickDramaCopy(DRAMA_DIALOGUE_BANK.rumour[rumour.kind], rumour.id), source, subject)
              : (rumour.claim ?? '').replaceAll(' ? ', ', ');
            const trail = rumour.originatorId === humanId
              ? chain.length > 1
            ...
            return (
              <article className="house-pulse__card house-pulse__card--intel" key={rumour.id}>
                <div className="house-pulse__card-top"><span>{RUMOUR_LABEL[rumour.kind] ?? 'House intel'}</span><em>{rumour.status === 'exposed' ? 'Public' : 'Unconfirmed'}</em></div>
                <h3>{subject}</h3>
                <p>{claim}</p>
                <small>{trail}</small>
              </article>
          ...
          {tab === 'history' && (knownEvents.length ? knownEvents.map((event) => (
            <article className={`house-pulse__card house-pulse__card--${event.severity}`} key={event.id}>
              <div className="house-pulse__card-top"><span>Day {event.week} - {PHASE_LABEL[event.phase] ?? event.phase.replaceAll('_', ' ')}</span><em>{event.public ? 'House-wide' : 'Private'}</em></div>
              <h3>{event.title ?? event.type.replaceAll('_', ' ')}</h3>
              <p>{event.text.replaceAll(' ? ', ', ')}</p>
              {event.detail && <small>{event.detail.replaceAll(' ? ', ', ')}</small>}
              {event.consequence && <strong className="house-pulse__fallout">Fallout: {event.consequence}</strong>}
            </article>
          )) : <p className="house-pulse__empty">Nothing important has happened on your radar yet.</p>)}
        </div>
      </section>
    </div>
  ) : null;
  ...
  return (
    <>
      <button type="button" className="house-pulse__summary" onClick={() => setOpen(true)}>
        ...
        <span><strong>House Pulse</strong><small>{activeStories} stories / {knownRumours.length} intel</small></span>
