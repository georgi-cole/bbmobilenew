...
function collectHealthIssues(state: RootState): HealthIssue[] {
  ...
  for (const nomineeId of game.nomineeIds) {
    if (!playerIds.has(nomineeId)) {
      ...
      issues.push({ severity: 'error', message: `Nominee ${nomineeId} is already out of the house.` });
    }
  }
    ...
    <>
      <section className="dbg-section">
        ...
        {issues.length > 0 && (
          <ul className="dbg-issue-list">
            {issues.map((issue, index) => (
              <li key={`${issue.message}-${index}`} className={`dbg-issue dbg-issue--${issue.severity}`}>
                {issue.message}
              </li>
            ))}
          </ul>
        ...
        <dl className="dbg-grid">
          <dt>Build</dt><dd>{import.meta.env.MODE}</dd>
          <dt>Online</dt><dd>{navigator.onLine ? 'yes' : 'no'}</dd>
          <dt>Viewport</dt><dd>{window.innerWidth}×{window.innerHeight} @ {window.devicePixelRatio}</dd>
          <dt>Last action</dt><dd>{actionHistory.at(-1)?.type ?? '—'}</dd>
          <dt>Last error</dt><dd>{lastError?.message ?? lastError?.reason ?? '—'}</dd>
        </dl>
      </section>
      ...
      <section className="dbg-section">
        ...
        <dl className="dbg-grid">
          <dt>RNG seed</dt><dd>{simulation.rng?.seed ?? 'not set'}</dd>
          <dt>RNG draw</dt><dd>{simulation.rng?.cursor ?? 0}</dd>
          <dt>Events</dt><dd>{reality.events.length}</dd>
          <dt>Interactions</dt><dd>{Object.keys(reality.interactions).length}</dd>
          <dt>Alliances</dt><dd>{Object.keys(reality.alliances).length}</dd>
          <dt>Romances</dt><dd>{Object.keys(reality.romances).length}</dd>
          <dt>Grievances</dt><dd>{Object.keys(reality.grievances).length}</dd>
          <dt>Trace entries</dt><dd>{simulation.trace.length}</dd>
          <dt>Last trace</dt>
          <dd>{latestTrace ? `${latestTrace.stage} · ${latestTrace.actionId ?? latestTrace.reason ?? 'no action'}` : '—'}</dd>
        </dl>
      </section>
      ...
      <section className="dbg-section">
        ...
        <p className="dbg-help">
          Re-enable in a published build with <code>?debug=1&amp;qa=1</code>. Press Ctrl+Shift+D
          to open or close this panel anywhere in the app.
        </p>
      </section>

      {status && <p className="dbg-status" role="status">{status}</p>}
    </>
  );
}
