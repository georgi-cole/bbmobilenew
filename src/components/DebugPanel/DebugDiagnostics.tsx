...
function collectHealthIssues(state: RootState): HealthIssue[] {
  ...
  if (game.replacementNeeded && game.nomineeIds.length < 1) {
    issues.push({
      severity: "warning",
      message:
        "A replacement nominee is requested without an existing nominee.",
    });
  }
...
export default function DebugDiagnostics() {
  ...
  const restoreSnapshot = (snapshot: DebugSnapshot) => {
    ...
    setStatus(
      `Restored snapshot from ${new Date(snapshot.exportedAt).toLocaleString()}.`,
    );
  };
  ...
  const handleSaveCheckpoint = () => {
    try {
      localStorage.setItem(
        CHECKPOINT_KEY,
        JSON.stringify(buildSnapshot(store.getState())),
      );
  ...
  const handleRestoreCheckpoint = () => {
    ...
    if (
      !window.confirm(
        "Replace the current campaign with the saved debug checkpoint?",
      )
    )
      return;
    try {
  ...
  const handleImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (
        !window.confirm(
          "Replace the current campaign with this imported debug snapshot?",
        )
      )
        return;
    ...
    <>
      ...
      <section className="dbg-section">
        ...
        <div className="dbg-row">
          <button
            className="dbg-btn dbg-btn--wide"
            onClick={() => navigate("/gamedebug")}
          >
            Open Minigame Auditor
          </button>
          <button
            className="dbg-btn dbg-btn--wide"
            onClick={() => navigate("/settingsatiste")}
          >
        ...
        <div className="dbg-row">
          ...
          <button
            className="dbg-btn dbg-btn--wide"
            onClick={() =>
              downloadJson("big-eye-reality-trace.json", simulation.trace)
            }
          >
        ...
        <p className="dbg-help">
          Re-enable in a published build with <code>?debug=1&amp;qa=1</code>.
          Press Ctrl+Shift+D to open or close this panel anywhere in the app.
        </p>
      </section>
