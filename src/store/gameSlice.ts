  ...
  reducers: {
    ...
    advance(state) {
      ...
      if (state.phase === 'final3') {
        // Hand off: reset week-level fields and start final competitions on next week
        state.week += 1;
        state.hohId = null;
        state.nomineeIds = [];
        ...
        state.phase = 'week_start';
        pushEvent(state, `Final 3 competitions begin! Week ${state.week} — the end is near. 🏆`, 'game');
        return;
      }
