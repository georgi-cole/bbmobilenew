...
describe('Battery Low responsive styles', () => {
  ...
  it('keeps hero reactions inside a fixed center-stage overlay', () => {
    ...
    expect(heroIcon).toContain("url('/assets/minigames/battery-low/hero-icons.webp')")
    expect(heroIcon).toContain('background-size: 400% 200%;')
    expect(css).toContain("url('/assets/minigames/battery-low/stage.webp')")
  ...
  it('uses a labelled Bank Offer sheet and removes the permanent ticker UI', () => {
    ...
    expect(source).toContain('HERO_REVEAL_SPRITES')
    ...
    expect(source).not.toContain('RevealSigil')
