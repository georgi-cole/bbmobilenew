...
describe('safe-area layout styles', () => {
  it('defines a shared top safe-area fallback and applies it in the app shell', () => {
    ...
    expect(navBarCss).toContain('padding-bottom: 0;');
    ...
    expect(minigameHostCss).toContain('var(--minigame-stage-top-padding)');
    expect(minigameRulesCss).toContain('calc(var(--fullscreen-safe-top) + 16px)');
    expect(quickTapCss).toContain('calc(16px + var(--safe-top))');
    expect(quickTapCss).toContain('calc(var(--fullscreen-safe-height) - 32px)');
