...
describe('safe-area layout styles', () => {
  ...
  it('restores HomeHub-owned full-height decorative background with safe controls', () => {
    ...
    expect(homeHubCss).toContain('.homehub-shell { position: relative; min-height: 100vh; min-height: 100dvh;');
    expect(homeHubCss).toContain('min-height: calc(100% + var(--app-safe-area-top, 0px) + var(--safe-bottom, 0px));');
    ...
    expect(homeHubCss).toContain('.homehub-frame { position: relative; width: min(100vw, 480px); height: 100vh; height: 100dvh;');
    expect(homeHubCss).toContain('padding-top: var(--app-safe-area-top, env(safe-area-inset-top, 0px));');
