...
describe('PublicFavoriteOverlay', () => {
  ...
  it('passes a public-opinion forecast into the live voting simulator', () => {
    ...
    expect(options?.targetPercentages?.p2).toBeGreaterThan(
      options?.targetPercentages?.p3 ?? 0,
    );
    expect(options?.targetPercentages?.p3).toBeGreaterThan(
      options?.targetPercentages?.p1 ?? 0,
    );
  ...
  it('makes the rewarded Viewer Spotlight explicitly cosmetic', async () => {
    ...
    expect(mockedUseBattleBackVoting.mock.calls.at(-1)?.[0]).not.toHaveProperty(
      'surgeTargetId',
    );
  });
  ...
  it('uses a readable fast-forward cadence instead of 260 ms eliminations', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Fast forward public favorite vote' }),
    );
  ...
  it('falls through directly to the reveal when one candidate is already complete', () => {
    ...
    render(
      <PublicFavoriteOverlay candidates={[PLAYERS[1]]} seed={41} onComplete={vi.fn()} />,
    );
